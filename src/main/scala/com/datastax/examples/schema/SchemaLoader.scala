  ...
  def loadSchema(g: JanusGraph, schemaFilePath: String): Unit = {
    ...
    try {
      ...
      graphSchema.vertexIndexes.foreach { vi =>
        if (!mgmt.containsGraphIndex(vi.name)) {
          val indexBuilder = mgmt.buildIndex(vi.name, classOf[Vertex])
          vi.propertyKeys.foreach(p => indexBuilder.addKey(mgmt.getPropertyKey(p)))
